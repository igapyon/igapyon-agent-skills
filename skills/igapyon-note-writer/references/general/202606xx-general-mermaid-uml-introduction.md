...
一方で、最近の開発ドキュメントは Markdown で書かれることが多くなってきたように思えます。README、設計メモ、仕様メモ、ADR、Issue、Pull Request の説明。どれも、テキストとして管理しやすいことが大事です。人にも生成AIにも扱いやすいテキスト形式であることが、この流れを少し後押ししているのかもしれません。

あの…この記事では、その変化そのものを主題にはしません。ただ、Markdown の中に Mermaid のコードブロックがふっと出てくることがあります。そのとき、書かれている図が何を表しているのかを読めると、設計メモを受け取る側としてかなり楽になります。

そこで今回取り上げるのが Mermaid です。Mermaid は、テキストで図を書ける記法です。Markdown の中にそのまま置けて、GitHub などの環境では図として表示できます。図を画像ファイルとして別管理するのではなく、設計メモの本文と同じ場所で管理できます。バイナリ画像ではないので、一般的な設計図ならファイルサイズも小さくなりやすく、git などで差分を見ながら、どこを直したのかをレビューしやすいところが嬉しいです。
...
この図では、Web 画面が API を呼び、API が注文サービスを呼び、注文サービスがデータベースを使う、という依存の向きが見えます。

## この図は、UMLではコンポーネント図に近い用途です

UML には **コンポーネント図** もあります。コンポーネント図は、システムを構成する部品と、その部品同士の関係を表す図です。

Mermaid の `flowchart` は、UML のコンポーネント図そのものではありません。でも、README や設計メモの中で、部品のつながりを簡単に説明したいときには、かなり使いやすい近似になります。
