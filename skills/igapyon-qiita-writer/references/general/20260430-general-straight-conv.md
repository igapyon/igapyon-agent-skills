...
---
title: [miku-soft] 生成AI時代のストレートコンバージョン設計
tags: mikuku 生成AI 移植 アーキテクチャ設計 AI駆動開発
author: igapyon
slide: false
---
## はじめに

`miku-soft` では、Node.js / TypeScript で作った上流実装を、Java CLI として利用できるようにすることがあります。
このとき、私は Java 版を Java-first に作り直すのではなく、まず `ストレートコンバージョン` として移す方針を取っています。
ここでいうストレートコンバージョンは、単純な機械翻訳ではありません。
上流実装のファイル境界、責務分割、語彙、CLI 契約、diagnostics、artifact role を、Java 側でも追跡できる形に移すことを重視します。
ストレートコンバージョン界隈に慣れておられる方には、このあたりはある意味で当たり前の作法かもしれません。
...
しかし、生成AI時代のソフトウェア開発として見直すと、この作法の意味が少し変わってきます。そもそも生成AIがあることで、ストレートコンバージョンにかかる実装コストが桁違いに軽くなっていることも大きな理由です。

![はじめに](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/105739/97b57e8f-4829-4808-a68a-5a30a2a27b3f.png)

ただし、大前提があります。

上流のソースコードが、ストレートコンバージョンに耐えられる程度には適切にリファクタリング済みであることです。上流側の責務分割やファイル境界がまだ大きく揺れている状態では、AI agent に渡す作業地図も不安定になります。

もう一つの大前提として、ソースコードなどの変更の一切は人手では行わず、生成AIに作業を委ねるということがあります。生成AI駆動開発を選択するのです。人間は、作業の座標系、制約、確認観点、判断基準を用意する側に回ります。

ストレートコンバージョンの設計は、AI agent が大きな移植作業を進めるための `作業地図` になります。

## ストレートコンバージョンで重視すること

`miku-soft` におけるストレートコンバージョンでは、最初の目的を「Java らしく美しく再設計すること」には置きません。
最初の目的は、上流の Node.js / TypeScript 実装を、Java 側で追跡可能な形に置くことです。
そのため、初期段階では次のような点を優先します。

- upstream file boundary を追えること
- upstream の責務分割を Java 側でも説明できること
- upstream の語彙や命名を Java 側から辿れること
- upstream の変更が来たとき、どこを比較すればよいか分かること
- Java 側の独自拡張と upstream 由来の契約を混ぜないこと

つまり、これは単なる移植ではなく、`upstream を追跡できる状態` を作る作業です。

私はこの性質を、英語のメモでは `upstream-following ability` と呼んでいます。

## 生成AI時代に景色が変わる

![生成AI時代に景色が変わる](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/105739/35958189-932b-4706-804f-cafb9e1c2872.png)

人間だけで移植するなら、対応関係や判断の背景は、ある程度人間の記憶や経験に置くことができます。
しかし、生成AIと一緒に移植する場合は、それだけでは足りません。
AI agent は、会話の途中ではよく働いてくれます。けれども、大きな移植作業では、次のような情報が会話の外に残っていることが重要になります。

- どの upstream file を見ればよいか
- どの Java class が対応しているか
- どの test が upstream のどの意図に対応しているか
- どの regression command を実行すべきか
- どの差分が Java 側の独自拡張なのか
- どの差分が upstream bug 由来なのか

これらが文書化されていると、AI agent は作業を再開しやすくなります。

逆に、ここが曖昧なまま AI に「Java 版をいい感じに移植して」と頼むと、見た目はそれらしい Java 実装が出てくるかもしれません。

しかし、それが upstream のどこに対応しているのか、次に upstream が変わったときにどこを直すべきなのか、判断しにくくなります。よくないパターンとしては、そもそもブロック単位で実装漏れが発生する場合もありました。

生成AI時代のストレートコンバージョンでは、AI の実装力を活かしつつ、自由に再設計しすぎないように座標系を先に固定することが大事になります。

## Java-first redesign を避ける

Java 版を作るとなると、つい Java らしく整理したくなります。
パッケージをきれいに分け直したり、抽象クラスや builder を入れたり、既存の Java ライブラリへ置き換えたりしたくなる場面もあります。どうしても Java美意識(?)が働きがちです。

それ自体が悪いわけではありません。

ただし、ストレートコンバージョンの初期段階でそれをやりすぎると、移植ではなく再実装に近づきます。そもそもその後の維持運用にも不安が残ります。

特に生成AIに任せる場合、「Java らしく」という指示は強すぎます。

AI は、もっともらしい Java 設計をかなり上手に作れます。  
しかし、その設計が upstream のファイル境界や責務分割を保っているとは限りません。

そのため、初期段階では次のように制約を置きます。

- Java-first redesign はしない
- upstream file boundary を尊重する
- upstream の関数名が `camelCase` なら、Java 側の method name も基本的に保つ
- class name は Java 慣習に寄せるが、upstream vocabulary は残す
- Java 側の独自拡張は別の契約として扱う

これは AI の能力を疑っているわけではありません。（とはいえ、私の観察では、現時点の生成AIはストレートコンバージョンを微妙に苦手としているようにも見えます）

AI が得意な実装作業を、追跡可能な範囲に閉じ込めるための設計です。

## mapping table は AI agent の作業地図になる

![mapping table は AI agent の作業地図になる](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/105739/d225791d-70ec-4ddd-aad2-1995a91cfb18.png)

ストレートコンバージョンでは、実装前または初期段階で mapping table を作ります。
たとえば次のようなものです。
...
しかし生成AI時代には、もう少し直接的な意味を持ちます。

AI agent にとって、mapping table は作業地図です。

```text
upstream file:
  vendor/example/src/ts/project-workbook-json.ts

java classes:
  jp.igapyon.example.projectworkbookjson.ProjectWorkbookJson
  jp.igapyon.example.projectworkbookjson.ProjectWorkbookJsonImport
  jp.igapyon.example.projectworkbookjson.ProjectWorkbookJsonExport
  jp.igapyon.example.projectworkbookjson.ProjectWorkbookJsonValidate

tests:
  ProjectWorkbookJsonUpstreamTest
```

このような対応があると、AI agent は次に読むべき場所を判断しやすくなります。
また、上流に変更が入ったときにも、「この upstream file の変更は、この Java class group とこの test に影響する」と説明しやすくなります。

これは、長い会話履歴に頼らず、リポジトリ内の文書で作業文脈を保つための仕組みでもあります。

## focused regression は確認単位になる

生成AIに移植作業を任せるとき、確認単位が曖昧だと作業がぶれやすくなります。
そこで、変更単位に対応する focused regression command を残します。
たとえば、ある upstream file と Java class group に対応する test や CLI smoke を明示しておきます。

```text
focused regression:
  mvn test -Dtest=ProjectWorkbookJsonUpstreamTest
```

このような確認単位があると、AI agent は「何を直したあと、何を実行すればよいか」を判断しやすくなります。
すべての変更で毎回フルテストを実行するのではなく、変更した責務に対応する最小の確認単位から始められます。

もちろん、最終的には `mvn test` や packaging の確認も必要です。

それでも、途中の作業では focused regression があることで、AI agent の試行錯誤がかなり安定します。

## follow-up log は会話の外に残す記憶になる

生成AIとの作業では、会話の中にかなり多くの判断が発生します。
しかし、その判断を会話だけに置くと、後から追いにくくなります。
そこで、ストレートコンバージョンでは follow-up log を残します。
...
生成AI時代には、会話の外に残す記憶として働きます。
AI agent が次回作業するとき、過去の判断、残っている差分、次に見るべき観点を読み取れます。

長い移植作業では、このような外部化された文脈がないと、再開のたびに同じ確認を繰り返したり、以前の判断を壊したりしやすくなります。

## byte-level parity は「だいたい同じ」を減らす

![byte-level parity は「だいたい同じ」を減らす](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/105739/71ec496d-4f39-4879-9fab-26f6407cc7e1.png)

ストレートコンバージョンでは、最初からすべてを byte-level parity で縛る必要はありません。

しかし、text / json / xml / zip / xlsx / report のような成果物では、byte-level parity がかなり有効です。
意味としては同じに見えても、実際には次のような差分が出ます。
...
これらは、人間が見れば軽微に見えることもあります。

しかし、AI agent にとっては、何を仕様として固定すべきか、何を許容差分として扱うべきかが曖昧になりがちです。

byte-level parity を導入すると、「だいたい同じ」という曖昧さを減らせます。
特に report や archive のような最終成果物では、早めに determinism と byte-level parity を確認しておく価値があります。

これにより、AI agent が無意識に Java 側の書式や順序を変えてしまうことを検出しやすくなります。

## Java 側の独自拡張は分けて扱う

ストレートコンバージョンといっても、Java 側に何も足さないわけではありません。
Java CLI として使いやすくするために、次のようなものを追加することがあります。
...
- Maven plugin
- Java runtime 向け diagnostics
- AI-facing prompt markdown の API / CLI retrieval contract

これらは実用上とても重要です。

ただし、upstream 由来の core contract と混ぜないようにします。
Java 側の独自拡張は、`Java-side original extensions` として分けて扱います。

この境界があると、AI agent も判断しやすくなります。

```text
これは upstream の移植なのか
これは Java runtime 向けの追加契約なのか
これは一時的な workaround なのか
```

この区別がないと、生成AIは便利そうな機能を core 側へ混ぜてしまうことがあります。
便利な拡張を足すこと自体はよいのですが、それが upstream-following ability を壊してしまうと、後の保守が難しくなります。

## ストレートコンバージョンの Tips

この記事では Node.js / TypeScript から Java への移植を題材にしています。

しかし、この話は Java 移植だけに閉じません。
生成AIと一緒に大きめのコード変換、再編、別 runtime 対応、CLI 化、ライブラリ化を進めるときにも、同じ考え方が使えます。

大事なのは、AI に丸ごと自由に任せるのではなく、作業の座標系を先に用意することです。
...
これらを明示しておくと、生成AIの作業はかなり安定します。

AI は実装を速く進められます。  
しかし、どの単位で比較し、何を守り、どこまで変えてよいかは、人間側が設計しておいたほうがよいです。

ストレートコンバージョンの作法は、そのためのよい型になります。

## おわりに

![おわりに](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/105739/261acf4a-3a18-4bd0-9c79-1bf576cd9d1c.png)

Node.js / TypeScript 実装を Java に移すストレートコンバージョンでは、従来から上流の責務分割、命名、テスト、成果物を追跡できることが重要でした。

生成AI時代には、この重要性がさらに増します。

mapping table、focused regression、follow-up log、byte-level parity、Java-side original extensions の分離は、人間のためだけの管理資料ではありません。
AI agent が迷わず作業し、途中から再開し、差分を説明し、確認すべき単位を選ぶための作業地図になります。

ストレートコンバージョンとは、単に別言語へ移植する作業ではありません。

生成AI時代には、AI の実装力を追跡可能な形で活かすための設計でもあるのだと思います。

## Appendix: 実際に使った全体プロンプト

![Appendix: 実際に使った全体プロンプト](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/105739/87a1184b-e3ab-40fc-9938-3cbaecaa854a.png)

ここには、実際にストレートコンバージョン作業で生成AIに渡した全体プロンプトを載せます。あるソフトを移植した時に本当に利用したものです。
※とても長いのですが、この分量が半自動的ストレートコンバージョンを実現するためには必要でした。ご参考までに。
