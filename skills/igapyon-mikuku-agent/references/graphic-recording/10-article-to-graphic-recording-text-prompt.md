...
共通の描画品質は [../style-contract.md](../style-contract.md) の
`mikuku-graphic-recording-v1` に従います。このプロンプトでは、記事固有の意味と
構図を決め、共通スタイルを別のデザインへ置き換えないでください。
...
- primary-relation: 画像の中心に置く関係、流れ、または対比
- secondary-relations: 主図を補足する関係（なければ `該当なし`）
- label-priority: 画像内に必ず残す語句を3〜6個
...
上記の出力に、次の節を必ず追加してください。値は記事内容によらず、原則として
共通スタイル契約から変更しません。
...
- 画面: 横長 3:2、左・中央・右の3領域、主図を最大化
- 紙面: 均一に明るい白〜ごく薄いアイボリー、淡い暖色の紙らしさ、濃色の読みやすい文字、やわらかい手描き線。暗い周辺減光や文字の背後の陰影なし
...
`グラレコ構図案` には、共通スタイルを適用したうえで、記事固有の構図を具体化してください。
吹き出しを描けない根拠が記事内にある場合だけ `吹き出し: 該当なし（理由）` とし、その理由を
`変更（deviation）` にも記録してください。理由のない「吹き出しなし」「みくくを小さくする」は
指定してはいけません。
...
`図解構造` の `layout-family` と `primary-relation` は、20番の最終プロンプトへ明記して
ください。画像生成モデルに主図の形式を再解釈させるために省略してはいけません。
